...
/**
 * Reusable admin CRUD manager. Drives the same UI as AdminResult for any
 * entity that exposes `${API}/<resource>/admin_page` plus standard REST routes
 * and accepts `{ <payloadKey>: { ...fields, <detailKey>_attributes, links_attributes } }`.
 *
 * config = {
 *   title, entityLabel,
 *   resource,            // path segment, e.g. 'admit_cards' or 'syllabus'
 *   payloadKey,          // e.g. 'admit_card'
 *   detailKey,           // e.g. 'admit_card_detail'
 *   hasLinks,            // boolean
 *   extraFields,         // [{ name, label }] additional top-level string fields
 * }
 */
const AdminEntityManager = ({ config }) => {
  const {
    title,
    entityLabel,
    resource,
  ...
  const emptyEntity = { title: '', category: '', date: '', ...Object.fromEntries(extraFields.map((f) => [f.name, ''])) }
  ...
  const loadItems = () => {
    setLoading(true)
    authFetch(`${API}/${resource}/admin_page`)
      .then((res) => res.json())
  ...
  const startEdit = async (item) => {
    setError('')
    try {
      const res = await authFetch(`${API}/${resource}/${item.id}`)
  ...
  const handleDelete = async (id) => {
    ...
    setError('')
    try {
      const res = await authFetch(`${API}/${resource}/${id}`, { method: 'DELETE' })
      ...
      {showForm && (
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {input('Application Fee', detailForm.application_fee, (v) =>
              setDetailForm({ ...detailForm, application_fee: v })
            )}
            ...
            {input('Download Link', detailForm.download_link, (v) =>
              setDetailForm({ ...detailForm, download_link: v })
            )}
