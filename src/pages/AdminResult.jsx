...
const AdminResult = () => {
  ...
  const loadResults = () => {
    setLoading(true)
    authFetch(`${API}/results/admin_page`)
      .then((res) => res.json())
  ...
  const startEdit = async (result) => {
    setError('')
    try {
      const res = await authFetch(`${API}/results/${result.id}`)
      ...
      const sourceLinks = Array.isArray(full.links)
        ? full.links
    ...
    <div className="p-3 sm:p-5">
      ...
      {showForm && (
        ...
        >
          <h2 className="mb-2 text-base font-semibold">
            {editingId ? `Edit Result #${editingId}` : 'Create Result'}
          </h2>
          ...
          <div className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
            {input('Title', resultForm.title, (v) =>
              setResultForm({ ...resultForm, title: v })
            )}
            {input('Category', resultForm.category, (v) =>
              setResultForm({ ...resultForm, category: v })
            )}
            {input(
              'Date',
              resultForm.date,
              (v) => setResultForm({ ...resultForm, date: v }),
              'date'
            )}
          </div>
          ...
          <div className="mt-1.5 space-y-2">
            {links.length === 0 && (
              <p className="text-[11px] text-gray-500">No links added.</p>
            )}
            {links.map((l, idx) => (
              <div
                key={idx}
      ...
      <div className="space-y-1.5 sm:hidden">
        ...
        {!loading && results.length === 0 && (
          <p className="py-3 text-center text-xs text-gray-500">No results found.</p>
        )}
        {results.map((r) => (
          <div
            key={r.id}
