...
const SyllabusDetails = () => {
  ...
  useEffect(() => {
    setLoading(true)
    fetch(`${import.meta.env.VITE_API_URL}/api/v1/syllabus/${id}`)
      .then(res => {
  ...
  useDocumentMeta(
    ...
    {
      type: 'article',
      canonical: `/syllabus/${id}`,
      jsonLd: item
        ? breadcrumb([
            { name: 'Home', url: '/' },
            { name: 'Syllabus', url: '/syllabus' },
            { name: item.title, url: `/syllabus/${id}` },
          ])
        : null,
    }
  )
  ...
  if (!item) return <p className="text-gray-500 text-sm">Syllabus not found.</p>
  ...
  const selectionProcess = item.selection_process
  const applicationFee = item.application_fee
  ...
  return (
    <div className="space-y-5">
      <Link to="/syllabus" className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900">
        <FaArrowLeft size={11} /> Back to Syllabus
      </Link>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                {category && (
                  <p className="mt-1 text-sm text-gray-500">{category}{item.exam ? ` · ${item.exam}` : ''}</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {date && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold tracking-wide px-2 py-1 rounded-full bg-purple-50 text-purple-700">
