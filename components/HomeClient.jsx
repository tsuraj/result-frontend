  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/jobs?${p.toString()}`)
      ...
      .then((data) => {
        ...
        // Refresh hero stats from the same response when available.
        if (data.meta) {
          setStats((prev) => ({
            ...prev,
            total_active: data.meta.total_active ?? data.meta.total ?? prev.total_active ?? 0,
            posted_today: data.meta.posted_today ?? prev.posted_today ?? 0,
            closing_this_week: data.meta.closing_this_week ?? prev.closing_this_week ?? 0,
          }))
        }
      })
      .catch(() => setJobs([]))
      ...
      <section className="bg-gradient-to-b from-red-50/60 to-transparent">
        <div className="container py-10 md:py-14">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-gray-600 mb-5">
            ...
            <button onClick={refreshStats} className="text-gray-400 hover:text-gray-700 underline-offset-2 hover:underline">refresh</button>
          </div>
                ...
                return (
                  <li key={label}>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-md text-sm ${active ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-50'}`}
                    >
                      {label}
                    </button>
                  </li>
            ...
            <div className="flex items-center justify-center gap-1 pt-4">
              ...
              {Array.from({ length: Math.min(3, totalPages) }, (_, i) => i + 1).map((n) => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  className={`px-3 py-1.5 rounded-md border text-sm ${page === n ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                >
                  {n}
                </button>
              ))}
              ...
              {totalPages > 3 && (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md border text-sm ${page === totalPages ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                >
                  {totalPages}
                </button>
              )}
        ...
        <aside className="lg:col-span-3 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/updates" className="text-xs text-gray-500 hover:text-gray-800">View all</Link>
            </div>
            {updates.length === 0 ? (
              ...
              <ul className="space-y-3">
                {updates.map((u, i) => {
                  ...
                  if (u.link_path) {
                    return <li key={u.id}><Link href={u.link_path} className="flex items-start gap-3 hover:opacity-80">{inner}</Link></li>
                  }
                  if (u.url) {
