...
export async function getJobs({ page = 1, perPage = 20, q, topic } = {}) {
...
// Helper: convert { topic: 'railway' } -> '?topic=railway' (empty values dropped).
const qs = (params = {}) => {
...
export const getResults = (params = {}) => listOrEmpty(`/results${qs(params)}`)
export const getResult = (slug) => getJSONOrNull(`/results/${slug}`)

export const getAdmitCards = (params = {}) => listOrEmpty(`/admit_cards${qs(params)}`)
export const getAdmitCard = (slug) => getJSONOrNull(`/admit_cards/${slug}`)

export const getAnswerKeys = (params = {}) => listOrEmpty(`/answer_keys${qs(params)}`)
export const getAnswerKey = (slug) => getJSONOrNull(`/answer_keys/${slug}`)

export const getSyllabi = (params = {}) => listOrEmpty(`/syllabus${qs(params)}`)
...
// Compact summary used by the home page CategoryGrid: just slug, name, total.
// Single round trip for all 5 topics.
export const getTopicsSummary = () => getJSONOrNull('/topics')
